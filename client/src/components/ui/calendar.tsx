...
const Calendar: React.FC<CalendarProps> = ({
      ...
      default: // 'days'
        ...
        onPrevClick = () =>
          handleViewDateChange(
            variant === "week" ? subWeeks(viewDate, 1) : subMonths(viewDate, 1)
          );
        onNextClick = () =>
          handleViewDateChange(
            variant === "week" ? addWeeks(viewDate, 1) : addMonths(viewDate, 1)
          );
        break;
    }
    ...
    return (
      <div className="flex items-center justify-between py-2 px-2">
        <button
          type="button"
          onClick={onPrevClick}
          className="p-2 rounded-md text-neutral-grey3 hover:bg-neutral-dark2/50 dark:hover:bg-neutral-grey1 focus:outline-none"
        >
          <ChevronLeft size={20} />
        </button>
        <button
          ...
          className="text-lg font-heading font-semibold text-neutral-dark1 dark:text-neutral-white hover:text-system-blue disabled:hover:text-current disabled:cursor-default"
        >
          {headerContent}
        </button>
        <button
  ...
  const renderDaysView = () => {
    ...
    const weekdays = Array.from({ length: 7 }).map((_, i) => (
      <div
        className="text-center font-medium text-sm text-neutral-grey3 py-2"
        key={i}
      >
        ...
        days.push(
          <button
            ...
            className={`relative p-2 h-12 w-full flex flex-col items-center justify-center rounded-lg transition-colors duration-200 group disabled:cursor-not-allowed ${
              ...
              isSelected
                ? "bg-primary-brand text-neutral-dark1 font-bold"
                : "hover:bg-neutral-softGrey2 dark:hover:bg-neutral-dark1"
            }`}
            key={day.toString()}
